...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $studentQuery = "SELECT RFID, Name, Student_Code, Email, Avatar FROM Student WHERE Student_Code = '$studentCode'";
    $studentResult = $conn->query($studentQuery);

    if ($studentResult->num_rows > 0) {
        $studentData = $studentResult->fetch_assoc();
        $rfid = $studentData["RFID"]; // Lấy RFID của sinh viên
        ...
        $attendanceHistoryQuery = "
            ...
            WHERE A.RFID = '$rfid' -- Sử dụng RFID ở đây
        ...
        $errorResponse = array("error" => "Student with Student Code $studentCode not found");
